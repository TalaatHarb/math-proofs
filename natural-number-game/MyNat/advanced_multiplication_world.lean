...
theorem mul_eq_zero_iff (a b : ℕ) :
  ...
  exact (
    Or.elim h 
    (fun ha => by 
    rewrite [ha, <-zero_equal_numeral ,zero_mul] rfl
    ) 
    (fun hb => by
    rewrite [hb, <-zero_equal_numeral, mul_zero] rfl
    )
    )
  }
...
theorem mul_left_cancel (a b c : ℕ) 
  ...
  | succ c' ih => 
    ...
    | succ b' _ =>
      rewrite [mul_succ, mul_succ] at habc
